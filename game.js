...
scene("game", ({ level, score}) => {
    ...
    const levelCfg = {
        ...
        '}': [sprite('skeletor'), 'dangerous', 'skeletor', 'skeletor-up', { dir: -1, timer: 0}],
        '.': [sprite('skeletor'), 'dangerous', 'skeletor', 'skeletor-side', { dir: -1}],
    ...
    action('skeletor-side', (s) => {
        s.move(s.dir * SKELETON_SPEED, 0)
        s.timer -= dt()
            if (s.timer <= 0) {
                s.dir = - s.dir
                s.timer = rand(5)
    ...
    collides('skeletor-up', 'wall', (s) =>{
        s.dir = -s.dir;
    })

    collides('skeletor-side', 'wall', (s) =>{
        s.dir = -s.dir;
    })
